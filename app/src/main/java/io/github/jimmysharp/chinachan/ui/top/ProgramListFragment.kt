package io.github.jimmysharp.chinachan.ui.top

import androidx.lifecycle.ViewModelProviders
import android.os.Bundle
import androidx.fragment.app.Fragment
import android.view.LayoutInflater
import android.view.View
import android.view.ViewGroup
import io.github.jimmysharp.chinachan.databinding.ProgramListFragmentBinding

class ProgramListFragment : Fragment() {

    companion object {
        fun newInstance() = ProgramListFragment()
    }

    private lateinit var binding : ProgramListFragmentBinding
    private lateinit var viewModel : ProgramListViewModel

    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?,
                              savedInstanceState: Bundle?): View {
        binding = ProgramListFragmentBinding.inflate(inflater, container, false)

        return binding.root
    }

    override fun onActivityCreated(savedInstanceState: Bundle?) {
        super.onActivityCreated(savedInstanceState)
        viewModel = ViewModelProviders.of(this).get(ProgramListViewModel::class.java)
        // TODO: Use the ViewModel
    }

}
